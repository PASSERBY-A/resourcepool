...
//                    },
//                    action: {
//                        custom: cloudStack.uiCustom.instanceWizard(cloudStack.instanceWizard02)
//                    },
//
//                    messages: {
//                        notification: function(args) {
...
//                },
            ...
            dataProvider: function(args) {
                ...
                if ("workOrders" in args.context) { //add @ma
 					var workItems = args.context.workOrders[0].workItems;
// 					if(workItems){
// 						$.each(workItems, function(index, value) {
// 							$.each(value,function(k,v){
// 								if(value.step==3 && "ID"==v && "attributeName"==k){
// 									$.extend(data, {
...
// 					}    
 					...
 					if(workOrder.workOrderJob[0])
 					$.extend(data,{
 						id:workOrder.workOrderJob[0].jobInstanceId
 					});
                ...
                actions: {
                    ...
                    destroy: {
                        ...
                        action: function(args) {                        	
                            ...
                            $.ajax({
                                url: createURL('customDestroyVirtualMachine&cmsz=yes'),
                                data: data,                                
                                success: function(json) {
                                	cloudStack.dialog.window({message:"实例删除成功"},function(){
                        ...
                        action: function(args) {
                        	$.ajax({
                                ...
                                success: function(data) {
                                	if(data.status==1){
                                		cloudStack.dialog.window({message:"已经为该实例提交更改服务方案工单，且处于待审批中，无法重复提交新工单"},function(){
                                			cloudStack.dialog.hide();
                                			//$(".resources form input[type='submit']").remove();
                                			return;
                                   	});
                                	}else if(data.status==2){
                                		cloudStack.dialog.window({message:"已经为该实例提交更改服务方案工单，且审批通过，等待处理中"},function(){
                                			cloudStack.dialog.hide();
                                			return;
                                   	});
                                	}else if(data.status==4){
                                		cloudStack.dialog.window({message:"已经为该实例提交更改服务方案工单，且正在处理中"},function(){
                                			cloudStack.dialog.hide();
                                			return;
                                   	});
                                	}else{
			                            $.ajax({
			                            	/**
			                            	 * updated by steven
			                            	 * command=scaleVirtualMachine改为saveOrder
			                            	 * 参数id改为适应需求的vmid
			                            	 * 新增传递的参数applyReason
			                            	 */
			                                url: createURL("saveOrder&vmid=" + args.context.instances[0].id + "&serviceofferingid=" + args.data.serviceOffering),
			                                dataType: "json",
			                                type:'post',
			                                data:{
                        ...
                        dataProvider: function(args) {
                            $.ajax({
                                ...
                                success: function(json) {
                                    ...
                                    if (isModuleIncluded("dr")) { //add on 4.4.1
