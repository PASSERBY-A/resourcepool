                ...
                listView: {
                    ...
                    detailView: {
                        ...
                        tabFilter: function(args) {
                        	var	account_type = args.context.accounts[0].accounttype;
                        	//alert(account_type);
                        	var hiddenTabs = [];
                        	if(!isAdmin()) {
                        		hiddenTabs.push('settings');
                        	}
                        	//@ma admin对其它非admin账户操作不需要进行申请直接修改即可 at 10-28
                        	if(isAdmin() && account_type!=1) {
                                    ...
                                    add: {
                                        ...
                                        action: function(args) {
                                        	...
                                        	if (!$("#validateForm").valid()) {
	                                       		 cloudStack.dialog.notice({message:"请在文本框内填入正确的数字"});
                                        	 ...
                                        	 $.ajax({
                                                 ...
                                                 data: {
                                                 	...
                                                 	account:args.context.accounts[0].name
                                                 },
                                                 type:'get',
                                                 dataType: 'json',
                                                 async: true,
                                                 success: function(data) {
                                                 	if(data.status==1){
                                                 		cloudStack.dialog.window({message:"该账户已经提交工单，且处于待审批中，无法重现提交新工单"},function(){
                                                 			cloudStack.dialog.hide();
                                                 			//$(".resources form input[type='submit']").remove();
                                                 			return;
                                                    	});
                                                 	}else if(data.status==2){
                                                 		cloudStack.dialog.window({message:"该账户已经提交工单，且审批通过，等待处理中"},function(){
                                                 			cloudStack.dialog.hide();
                                                 			return;
                                                    	});
                                                 	}else if(data.status==4){
                                                 		cloudStack.dialog.window({message:"该账户已经提交工单，且正在处理中"},function(){
                                                 			cloudStack.dialog.hide();
                                                 			return;
                                                    	});
                                                 	}else{
			                                        	 args.data=cloudStack.serializeForm($("#validateForm"));
			                                        	 $.ajax({
			                                                 ...
			                                                 data: {
			                                                 	...
			                                                 	status:1,
			                                                 	accountId: args.context.accounts[0].id,
			                                                     domainId: args.context.accounts[0].domainid,
			                                                     account:args.context.accounts[0].name,
			                                                     instance:args.data['vmLimit'],
			                                                     ip:args.data['ipLimit'],
			                                                     volume:args.data['volumeLimit'],
			                                                     snapshot:args.data['snapshotLimit'],
			                                                     template:args.data['templateLimit'],
			                                                     network:args.data['networkLimit'],
			                                                     vpc:args.data['vpcLimit'],
			                                                     cpu:args.data['cpuLimit'],
			                                                     memory:args.data['memoryLimit'],
			                                                     primaryStorage:args.data['primaryStorageLimit'],
			                                                     secondaryStorage:args.data['secondaryStorageLimit'],
