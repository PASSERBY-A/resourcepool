            ...
            var instanceWizard = function(data) {
                ...
                var dataGenerators = {
                    ...
                    'review': function($step, formData) {
                        $step.find('[wizard-field]').each(function() {
                            ...
                            /*add @ma*/
                            if(field=="apply-time-length"){
                            	fieldName = $wizard.find('div.setup div.apply-time-length input[type=text]').val();
                            	if(fieldName)
                            		fieldName = fieldName + "  月";
                            }
                            if (fieldName) {
                                $(this).html(fieldName);
                            } else {
                ...
                var showStep = function(index, options) {
                    ...
                    if ($targetStep.hasClass('review')) {
                    	...
                    	$nextButton.find('span').html(_l('提交申请'));
                ...
                //add by@ma
                var minApplyMonths =1;
                var maxApplyMonths =120;
                $wizard.find('.section.apply-time-length .size.min span').html(minApplyMonths);
                $wizard.find('.section.apply-time-length .size.max span').html(maxApplyMonths); //end
                ...
                //add by @ma
                $wizard.find('.section.apply-time-length .slider').each(function() {
                    var $slider = $(this);
                     ...
                     $slider.slider({
                         ...
                         max: maxApplyMonths,
//                         start: function(event) {
//                             $slider.closest('.section.apply-time-length').find('input[type=radio]').click();
//                         },
                         slide: function(event, ui) {
                             $slider.closest('.section.apply-time-length').find('input[type=text]').val(
                                 ui.value
                             );
//                             $slider.closest('.step').find('span.custom-disk-size').html(
//                                 ui.value
