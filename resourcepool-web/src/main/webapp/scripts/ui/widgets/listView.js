...
(function($, cloudStack, _l, _s) {
    ...
    var createHeader = function(preFilter, fields, $table, actions, options) {
        ...
        if (detailView && !$.isFunction(detailView) && !detailView.noCompact && !uiCustom) {
        	//添加@ma 10-20 添加判断如果detailview属性为false不显示快速查看 
        	if(detailView.quickView!=false){
        		$thead.find('tr').append(
        				$('<th></th>')
    ...
    var createSearchBar2 = function($toolbar, listViewData) {
        var $search = $('<div></div>').addClass('text-search reduced-hide');
       // var $searchBar = $('<div></div>').addClass('search-bar reduced hide').appendTo($search);
       // $searchBar.append('<input type="text" />');
       // $search.append('<div id="basic_search" class="button search"></div>');
...
//                            .addClass('button')
                            .attr({
                                type: 'button',
                                value: _l(field.label)
                            })
                            .click(function(){
                            	if (field.action) {
                            		field.action(dataItem, $tr);
                            	}
                            })
                        );
                }
          ...
          // Add sub-select
            ...
            // Add quick view
            if (detailView && !$.isFunction(detailView) && !detailView.noCompact && !uiCustom) {
            	if(detailView.quickView!=false){  //添加对快速查看一个判断@ma 10-20
    ...
    var makeListView = function($container, args, section) {
        ...
        createFilters($toolbar, listViewData.filters);
         if(listViewData.hideToolbar2==true){
        	 createSearchBar2($toolbar, listViewData);
         }else if (listViewData.hideSearchBar != true) {
        ...
        $listView.bind('click change', function(event) {
            ...
            //--------------added 10/09  追 加的列操作标识----------------------------------------------------//          
//            var appendAction =  ($target.closest('div.data-table tr td').size() &&
//                    ($target.closest('div.data-table tr td').index() == 0 || ($target.closest('div.data-table tr td').attr('class')=='appendAction' && $target.closest('div.data-table tr td').html()!=''))&&
//                    listViewData.detailView && !$target.closest('div.edit').size());
//           
//            if (appendAction && !uiCustom && !$target.closest('.empty, .loading').size()) {
//            	
//          	  var $loading = $('<div>').addClass('loading-overlay');
//              $target.closest('div.data-table').prepend($loading); //overlay the whole listView, so users can't click another row until click-handling for this row is done (e.g. API response is back)
//              listViewData.appendView.$browser = args.$browser;
//           
//             
//             
//              var listViewArgs = $listView.data('view-args');
...
//              );*/
//              
//              console.info(listViewData);
//              
//              // Get path in cloudStack args
//              var viewAllPath = listViewData.appendView.viewAll.path.split('.');
//              
...
//              var customTitle =_l(listViewArgs.title);
//              // Make list view
...
//              else
//                  listViewArgs.id = listViewData.appendView.viewAll.path;
//
//              listViewArgs.ref = {
//                  //id: args.id
//            		  id: id
...
//        return $listView.appendTo($container);
        ...
        if (args && args.listView && args.listView.afterFilter) {
        	args.listView.afterFilter();
        }       
        return $container;
    };
