 ...
 *  version:2012-11-21
 *  author: MuZhongLiang
 *  desc:update check status after set value
 *  用法:
...
Ext.define('Ext.ux.ComboBoxTree',{
	...
	createPicker:function(){
		...
		self.picker = Ext.create('Ext.tree.Panel',{
			...
			useArrows : true,
			store : self.store,
			rootVisible : self.rootVisible==null?false:self.rootVisible,
			listeners:{
				select : function (selModel, record, index, options){
					//try{
						selectTreeNode(selModel);
					//}catch(error){}
				}
			},
			viewConfig: {
	...
	setDefaultValue:function(submitValue,displayValue){
		...
		this.pathArray = [];
		if(this.multiSelect){//如果多选遍历选择节点
			var ids = submitValue.split(",");
			for(var i in ids){
				var treeNode = this.store.getNodeById(ids[i]);
				if(treeNode) treeNode.set('checked',true);
			}
		}
	},
