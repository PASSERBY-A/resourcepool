...
<ul id="tabs">
	<li><a href="#" name="tab1">告警</a></li>
    <li><a href="#" name="tab2">KPI 列表</a></li>
    <li><a href="#" name="tab3">KPI 仪表盘</a></li>
</ul>

<div id="content"> 
    <div id="tab1">
		<iframe id="alert" width="100%" frameborder="0" scrolling="no" marginwidth="0" marginheight="0"
		 src="http://<%=ip%>:<%=port%>/<%=project%>/pages/dashboard/alarmSearch/index.jsp?a=1&moId=&mo=&agentId=&theme=-gray&browserLang=zh_CN&usr=<%=user%>&pwd=<%=pwd%>&kpiCode=<%=kpicode%>&ciName=<%=ciname%>"></iframe>
    </div>
    <div id="tab2">
    ...
    $('#tabs a').click(function(e) {
        ...
        if ($(this).closest("li").attr("id") == "current"){ //detection for current tab
         return;       
          ...
          if ($(this).attr('name') == "tab2" && $("#kpi").attr('src') == "") {
              var url2 = "http://<%=ip%>:<%=port%>/<%=project%>/dashboard/kpilist/index?a=1&moId=1Fk8Nr6Uzd8WB4In1Qu9XCg5Jo2RvaY&mo=1Fk8Nr6Uzd8WB4In1Qu9XCg5Jo2RvaY&agentId=&theme=-gray&browserLang=zh_CN&usr=<%=user%>&pwd=<%=pwd%>&kpiCode=<%=kpicode%>&ciName=<%=ciname%>";
              $("#kpi").attr('src',url2);
          }
          if ($(this).attr('name') == "tab3" && $("#kpid").attr('src') == "") {
              var url3 = "http://<%=ip%>:<%=port%>/<%=project%>/pages/dashboard/mainEngine/index.jsp?a=1&moId=xovm06-16.159.216.106&mo=xovm06-16.159.216.106&agentId=&theme=-gray&browserLang=zh_CN&usr=<%=user%>&pwd=<%=pwd%>&kpiCode=<%=kpicode%>&ciName=<%=ciname%>";
              $("#kpid").attr('src',url3);
          }
        }
    });
