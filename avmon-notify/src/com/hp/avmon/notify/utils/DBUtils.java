...
public class DBUtils {
    ...
    public static synchronized JdbcTemplate getJdbcTemplate() {
        if(jdbcTemplate==null){
            jdbcTemplate=(JdbcTemplate) SpringContainer.getBean("jdbcTemplate");
        }
        return jdbcTemplate;
    }
...
//    }
    ...
    public static String getDBCurrentDateFunction(){
    	String jdbcDriver = PropUtil.getString("config","connection.driver_class");
    	
        if(jdbcDriver.indexOf("oracle")>=0){
            return "sysdate";
        }
        else if(jdbcDriver.indexOf("postgresql")>=0){
            return "now()";
        }
        else{
            return "currentdate()";
    ...
    public static boolean isOracle(){
    	String jdbcDriver = PropUtil.getString("config","connection.driver_class");
        return jdbcDriver.indexOf("oracle")>=0;
    }
    
    public static boolean isPostgres(){
    	String jdbcDriver = PropUtil.getString("config","connection.driver_class");
        return jdbcDriver.indexOf("postgresql")>=0;
    }
