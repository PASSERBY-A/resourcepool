...
Ext.application({
    ...
    launch: function() {
		Ext.virtualMachine.application = this;
		Ext.virtualMachine.application.loadAll();
            },

    loadAll: function() {
            	Ext.virtualMachine.application = this;

                var moId=Ext.virtualMachine.moId;
    ...
    loadCourse: function() {
        if (Ext.virtualMachine.moId != null && Ext.virtualMachine.moId != '') {

            Ext.Ajax.request({
                url: 'courseUse',
                params: {mo:Ext.virtualMachine.moId},
        ...
        if (Ext.virtualMachine.moId != null && Ext.virtualMachine.moId != '') {
            Ext.Ajax.request({
                ...
                success: function(response, opts) {
                    ...
                    Ext.getCmp('gaugePanel').body.update('<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="../virtualMachine/gauge.jsp?cpu_user='+cpu_user+'&level='+cpu_level+'"></iframe>');
                },
                failure: function(response, opts) {
                    if(console && console.log){
                        console.log("load dashboard-mem error!");
    ...
    loadDisk: function() {
        if (Ext.virtualMachine.moId != null && Ext.virtualMachine.moId != '') {
            ...
            activeStoreProxy.extraParams.moId = Ext.virtualMachine.moId;
    ...
    loadBasicInfo: function() {
        ...
        if (moId != null && moId != '') {
            ...
            var activeStoreProxy = activeGrid.getStore().getProxy();
            activeStoreProxy.extraParams.treeId = Ext.virtualMachine.moId;
            activeGrid.getStore().load();
