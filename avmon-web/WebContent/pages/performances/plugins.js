...
//	}
    ...
    setTimeout(function(){
	item.panel.update('<iframe id="iframe-'+item.id+'" scrolling="auto" frameborder="0" width="100%" height="100%" src="'+item.url+'&moId='+item.newMoId+'&mo='+item.newMoId+'"></iframe>',
			false,function(){
				setTimeout(function(){
						Ext.MessageBox.hide();
						item.initFlag=1;
...
//	if(showingItem.newMoId.indexOf('@') > -1)
//	{
//		var segs = showingItem.newMoId.split('@');
//		//console.log("segs : "+segs);
//		showingItem.newMoId = segs.pop();
//	}
	//刷新当前页面
...
//	return;
//	
//	var parentNode=Ext.getCmp("performance-menuTree").getSelectionModel().getSelection()[0];
//
//	parentNode.expand(false,function(){
//	    Ext.getCmp("performance-menuTree").getSelectionModel().select(Ext.getCmp("performance-menuTree").getStore().getNodeById(moId),true);
//
//	});
}
