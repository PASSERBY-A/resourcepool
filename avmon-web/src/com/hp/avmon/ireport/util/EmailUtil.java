...
public class EmailUtil {
	...
	// smtp服务器
	private String host = "smtp3.hp.com";
	// 授权信息
	...
	private String pwd = "";
	// 收件人地址 
	private String to = "zhongliang.mu@hp.com";
	// 邮件标题
	private String subject = "";
    // 送信人
    private static String MAIL_FROM = "zhongliang.mu@hp.com";
	...
	public static void setMAIL_FROM(String mAIL_FROM) {
		MAIL_FROM = mAIL_FROM;
	}
    ...
    public EmailUtil() {
    	List<HashMap<String, Object>>  mailHost = this.getJdbc().queryForList("SELECT id, email_host as \"emaiHost\", email_port as \"emailPort\", email_from as \"emailFrom\","
    			+ " email_from_pwd  as \"emailFromPwd\" FROM ireport_email_host where id=1");
    	if(mailHost.size()>0){
    		Map<String, Object> mailMap = mailHost.get(0);
    		this.setHost(String.valueOf(mailMap.get("emaiHost")));
    		this.setUser(String.valueOf(mailMap.get("emailFrom")));
    		this.setPwd(String.valueOf(mailMap.get("emailFromPwd")));
    		setMAIL_FROM(String.valueOf(mailMap.get("emailFrom")));
    	}
    }
	
	
    public void autoSend(String txt, HashMap<String, Object> map, Map<String,String> image, Map<String,String> files) {
        ...
        try {
            ...
            if(files!=null && files.size()>0) {  
                Set<Entry<String, String>> set=files.entrySet();  
                for (Iterator iterator = set.iterator(); iterator.hasNext();) {  
                   Entry<String, String> entry = (Entry<String, String>) iterator.next();  
                    
                   System.out.println(">>>>>>>>>>>>>>>>>>>>>file name >>>>>>>>>>>>>>" + entry.getKey());
                   System.out.println(">>>>>>>>>>>>>>>>>>>>>file path>>>>>>>>>>>>>>" + entry.getValue());
                   
                   //添加附件
                   String attPath = entry.getValue();//"C:/mediaReport/template/test1/TestReport测试.pdf"; //附件地址
                   String attName = entry.getKey();//"TestReport测试.pdf"; //附件名称

                   BodyPart messageBodyPart= new MimeBodyPart();
                   FileDataSource source = new FileDataSource(attPath);
                   //添加附件的内容
                   messageBodyPart.setDataHandler(new DataHandler(source));
                   //添加附件的标题
                   //这里很重要，通过下面的Base64编码的转换可以保证你的中文附件标题名在发送时不会变成乱码
                   sun.misc.BASE64Encoder enc = new sun.misc.BASE64Encoder();
                   messageBodyPart.setFileName("=?GBK?B?"+enc.encode(attName.getBytes())+"?=");
    ...
    public MimeBodyPart createContent(String body, Map<String,String> image)throws Exception{  
        ...
        if(image!=null && image.size()>0) {  
             ...
             for (Iterator iterator = set.iterator(); iterator.hasNext();) {  
                ...
                System.out.println(">>>>>>>>>>>>>>>>>>>>>file cid >>>>>>>>>>>>>>" + entry.getKey());
                System.out.println(">>>>>>>>>>>>>>>>>>>>>file path>>>>>>>>>>>>>>" + entry.getValue());
