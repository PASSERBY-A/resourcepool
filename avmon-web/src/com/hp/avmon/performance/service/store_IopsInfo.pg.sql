...
* from
(select 
    CAST(coalesce(VALUE, '0') as integer) AS XP_CLP_MS, PATH
...
left join 
(select 
    CAST(coalesce(VALUE, '0') as integer) AS XP_CLP_CMUS, PATH
from (select a.mo_id,a.instance,a.kpi_code,b.* from tf_avmon_kpi_value_list a,tf_avmon_kpi_value_current b where a.value_key=b.value_key) k 
where mo_id='%s' and KPI_CODE='xp_clp_cmus' and AMP_INST_ID = 'wk_xpclp_1007') b on a.PATH = b.PATH
left join 
(select 
    CAST(coalesce(VALUE, '0') as integer) AS XP_CLP_CMWR, PATH
from (select a.mo_id,a.instance,a.kpi_code,b.* from tf_avmon_kpi_value_list a,tf_avmon_kpi_value_current b where a.value_key=b.value_key) k 
