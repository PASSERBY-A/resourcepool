...
from STG_AVMON_AVAILABLE_SUMMARY a 
	...
	inner join (
		select mo_Id from TF_AVMON_ALARM_DATA where KPI_CODE='1001011005' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate
		and mo_id not in (select mo_id from TF_AVMON_ALARM_DATA where KPI_CODE='sys_idle' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate-1/12)
		group by mo_id
		union
		select mo_Id from TF_AVMON_ALARM_DATA where KPI_CODE='1001010001' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate
		and mo_id not in (select mo_id from TF_AVMON_ALARM_DATA where KPI_CODE='1001010001' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate-1/12)
		group by mo_id
		union
		select mo_Id from TF_AVMON_ALARM_DATA where KPI_CODE='1001013003' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate
		and mo_id not in (select mo_id from TF_AVMON_ALARM_DATA where KPI_CODE='1001013003' and current_grade=5 and first_occur_time between sysdate-1/12 and sysdate-1/24/2)
		group by mo_id
		union
		select mo_Id from TF_AVMON_ALARM_DATA where KPI_CODE='sys_log' and current_grade=5 and first_occur_time between sysdate-1/24/2 and sysdate
		and mo_id not in (select mo_id from TF_AVMON_ALARM_DATA where KPI_CODE='sys_log' and current_grade=5 and first_occur_time between sysdate-1/12 and sysdate-1/24/2)
		group by mo_id
		) f on a.id=f.mo_id
