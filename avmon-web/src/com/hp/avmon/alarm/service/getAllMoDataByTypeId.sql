...
order by tt.type_id) 
%s 
group by "mo","attr_ipaddr","typeId" 
union

select 
"mo" ,"attr_ipaddr","typeId",
...
left join td_avmon_mo_info_attribute t4 on (a.mo_id=t4.mo_id and t4.name='hostName') 
where b.alarm_id is null 
%s
) a
group by "mo","attr_ipaddr","typeId"