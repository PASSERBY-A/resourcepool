...
left join (select id,to_char(last_update_time,'YYYY-MM-DD HH24:MI:SS') as last_update_time, case when last_update_time>(localtimestamp - interval '1 hour') then 1 else 0 end as status from STG_AVMON_AVAILABLE_SUMMARY) b on a.id=b.id 
left join (
