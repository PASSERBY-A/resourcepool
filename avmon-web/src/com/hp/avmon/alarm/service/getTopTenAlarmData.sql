...
end) as "value"  from TD_AVMON_MO_INFO a inner join (select count(*) total, mo_id 
...
--inner join (select value,mo_id from td_avmon_mo_info_attribute where name='businessSystem' and value='%s')
--d on a.mo_id = d.mo_id  
%s
