package com.hp.avmon.oidmanage.web;
 ...
 */
@Controller
@RequestMapping("/oidmanage/*")
public class OidmanageController {

	private static final Logger logger = LoggerFactory
			.getLogger(OidmanageController.class);
	...
	public String index(Locale locale, Model model) {
		...
		return "/config/snmp/oidmanage/index";
	...
	public void getMoDetail(HttpServletRequest request, PrintWriter writer)
		...
		Map mo = snmpDeployService.getMoDetail_oidmanage(moId, request);
	...
	public void deleteMo(HttpServletRequest request, PrintWriter writer)
		...
		String moId = request.getParameter("mo");
		// moId="201209112222472247";
		Map mo = snmpDeployService.deleteMo_oidmanage(moId);
		// to-anyi:请在这里将list处理成带层级的json格式
		String json = JackJson.fromObjectToJson(mo);
	...
	public void createMo(HttpServletRequest request, PrintWriter writer)
		...
		String instanceNumbers = request.getParameter("instanceNumbers");
		//businessType = AgentTypes.BIZMAP.get(businessType);
		businessType = this.snmpService.getBusinessNameById(businessType);
		// moId="201209112222472247";
		Map mo = snmpDeployService.createMo_oidmanage(moId, moType, moCaption, agentId,businessType,instanceNumbers, request);
		// to-anyi:请在这里将list处理成带层级的json格式
		String json = JackJson.fromObjectToJson(mo);
	...
	public void getMenuTree(HttpServletRequest request, PrintWriter writer)
		...
		if ("1".equals(queryFlag)) {
			json = snmpDeployService.getMoTreeJsonByNameOidmanage(userId,parentId);
		}else{
			...
			//add by fengjiawei start
			json = snmpDeployService.getMoTreeJsonOidmanage(userId, parentId,businessType);
			//add by fengjiawei end
		}
	...
	public void saveMoProperty(HttpServletRequest request, PrintWriter writer)
		...
		Map mo = snmpDeployService.saveMoProperty_oidmanage(moId, props);
		// to-anyi:请在这里将list处理成带层级的json格式
		String json = JackJson.fromObjectToJson(mo);
