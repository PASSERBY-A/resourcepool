...
public class AgentManageService {
	...
	private static final Log logger = LogFactory.getLog(AgentManageService.class);
    ...
    private static final String NOT_EXISTS_DATE = "1 1 1 30 2 1";
...
//        System.out.println("page is >>>>>>>>>>>>>>>>>>>>:"+page);
        ...
        if(sortBy!=null){
			...
			}else if("updateStatus".endsWith(sort)){
				sort = "round((sysdate - ag.last_update_time) * 24 * 60)";
				if(DBUtils.isPostgreSql())
					sort = "round((EXTRACT(EPOCH from CURRENT_TIMESTAMP) - EXTRACT(EPOCH from ag.last_update_time)) / 60)";
			}else if("lastHeartbeatTime".endsWith(sort)){
			    sort = "ag.last_heartbeat_time";
			}else if("lastUpdateTime".endsWith(sort)){
			    sort = "ag.last_update_time";
			...
			orderBy = " order by " + sort + " " + jsonArr.getJSONObject(0).get("direction")+", ag.AGENT_ID";
		}
        ...
        if(!"666656".equals(businessType)){
        	listSql = SqlManager.getSql(AgentManageService.class, "getBizAgentGridInfo");
        	countSql = SqlManager.getSql(AgentManageService.class, "getAgentGridInfoCount");
        	if(null != businessType){
            	//businessType = AgentTypes.BIZMAP.get(businessType);
        		businessType = equipmentCenterService.getBusinessNameById(businessType);
        	...
        	listSql = SqlManager.getSql(AgentManageService.class, "getBizAgentGridInfo_other");
        	countSql = SqlManager.getSql(AgentManageService.class, "getAgentGridInfoCount_other");
        }
    ...
    public Map findAgentAmp(HttpServletRequest request){
        ...
        String listSql = SqlManager.getSql(AgentManageService.class, "findAgentAmpList");
        String finSql =  generatPageSql(listSql+where,limit,start);

        String countSql = SqlManager.getSql(AgentManageService.class, "findAgentAmpListCount")+where;
...
//			String returnString = transfer1(batchSql);
    ...
    public Map getNormalAmpAttrList(HttpServletRequest request){
        Map<String, Object> map = new HashMap<String, Object>();
        String ampId = request.getParameter("ampId");
        String sql = String.format(SqlManager.getSql(AgentManageService.class, "getNormalAmpAttrList")+" where a1.AMP_ID = '%s' order by a1.ORDER_INDEX asc",ampId);
    ...
    public Map pushAgentAmpScript(HttpServletRequest request){
        ...
        for(Map<String,String> ampMap : ampListMapF){
        	...
        	if(result.startsWith("00")){
        		// modify by mark start
                //下发成功后修改AMP状态为  --正在运行运行
        		String updateAmpStatusSql = String.format(SqlManager.getSql(this, "updateAmpStatus"),ampInstId,ampId,agentId);
        		// modify by mark start
...
//        }
    ...
    public Map pushAgentAmpConfig(HttpServletRequest request){
        ...
        boolean flag = false;
        if(null == ampListMapF.get(0)){
        	ampListMapF = (ArrayList) request.getAttribute("agentAmpInfo");
        	logger.debug("===========ampListMapF===================="+ampListMapF);
        }
        Locale locale = request.getLocale();
        ResourceBundle bundle = ResourceBundle.getBundle("messages",locale);
        //TODO 调用接口下发配置
        for(Map<String,String> ampMap : ampListMapF){
        	...
        	String sql = String.format(SqlManager.getSql(this, "pushAgentAmpConfig"),ampInstId,agentId);
        	jdbcTemplate.execute(sql);
        	}else {
        		logger.error("下发失败"+result);
            	map.put("success",false);
            	map.put("msg", result);
            }
        }
    ...
    public Map getNormalAmpSchedule(HttpServletRequest request){
        ...
        jdbcTemplate.execute(sql);
        //返回查询结果
        sql = SqlManager.getSql(AgentManageService.class, "getNormalAmpSchedule");
    ...
    public Map pushAmpSchedule(HttpServletRequest request){
        ...
        */
        ...
        String selectedids = "";
        String ampInstId = "";
        ampInstId = mapList.get(0).get("ampInstId");
        for(Map<String,String> ampMap : mapList){
        	String kpiCode = ampMap.get("kpiCode");
        	selectedids+=kpiCode+"','";
        }
        
        selectedids = selectedids.substring(0,selectedids.length()-3);
        //1.查询所有kpicode对应的 group的SCHEDULE
    	//2.循环遍历所有的group,更新同组的kpiCode的调度策略为相同调度策略
    	String allScheduleSql = "select distinct SCHEDULE as \"schedule\",KPI_GROUP as \"kpiGroup\" from td_avmon_amp_policy where agent_id = '"+agentId+"' and AMP_INST_ID = '"+ampInstId+"' and kpi_code in ('"+selectedids+"')";
    	@SuppressWarnings("unchecked")
		final List<Map<String,String>> schedules = jdbcTemplate.queryForList(allScheduleSql);
    	String kpiGroups = "";
    	
    	for (Map<String, String> map1 : schedules) {
    		kpiGroups+=map1.get("kpiGroup")+"','";
		}
    	
    	if(schedules.size() > 0){
    		kpiGroups = kpiGroups.substring(0,kpiGroups.length()-3);
    		boolean updateResult = this.changePolicySchedule(agentId, ampInstId, schedules,kpiGroups);
            ...
            if(updateResult){
        		...
        		if(result.startsWith("00")){
        			...
        			this.updatePolicyStatus(agentId,ampInstId,kpiGroups);
        		}else{
     ...
     * @return
     */
    private boolean changePolicySchedule(String agentId, String ampInstId,final List<Map<String,String>> schedules,String kpiGroups) {
    	boolean result = false;
    	
    	String updateSql = "update TD_AVMON_AMP_POLICY set SCHEDULE = ? where agent_id = '"+agentId+"' and AMP_INST_ID = '"+ampInstId+"' and KPI_GROUP = ?";
    	String updateOtherSql = "update TD_AVMON_AMP_POLICY set SCHEDULE = '"+NOT_EXISTS_DATE+"' where agent_id = '"+agentId+"' and AMP_INST_ID = '"+ampInstId+"' and KPI_GROUP not in ('"+kpiGroups+"')";
		...
		if (cnt.length > 0) {
			 try {
				...
				logger.error(e);
				logger.error("========updatePolicySchedule===========更新其他policy失败");
				return false;
			}
		}
	...
	/**
     * 将下发的调度策略状态更新为1,将未选中的调度策略修改为0
     * @param selectedids
     */
    private boolean updatePolicyStatus(String agentId,String ampInstId,String kpiGroups) {
    	boolean result = false;
    	String updateSelectedSql = "update TD_AVMON_AMP_POLICY set STATUS = '1' where agent_id = '"+agentId+"' and amp_inst_id = '"+ampInstId+"' and schedule !='"+NOT_EXISTS_DATE+"' and kpi_group in ('";
    	String updateNonSelectedSql = "update TD_AVMON_AMP_POLICY set STATUS = '0' where agent_id = '"+agentId+"' and amp_inst_id = '"+ampInstId+"' and kpi_group not in ('";
    	if(!"'".equals(kpiGroups)){
    		updateSelectedSql+=kpiGroups+"')";
    		updateNonSelectedSql += kpiGroups+"')";
    		try {
    			jdbcTemplate.execute(updateSelectedSql);
        		jdbcTemplate.execute(updateNonSelectedSql);
        		result = true;
			} catch (Exception e) {
				e.printStackTrace();
				logger.error(e);
				logger.error("===========updatePolicyStatus更新失败=======");
			}
    	}
    	return result;
	}
    ...
    public Map saveAgentSchedue(HttpServletRequest request){
        ...
        if(jsonMap.get("kpiGroup") == null || jsonMap.get("kpiGroup").equals("")){
           String sql = "update TD_AVMON_AMP_POLICY set SCHEDULE = ? where KPI_CODE = ? and AGENT_ID = ? and AMP_INST_ID = ? and KPI_GROUP is null ";
           jdbcTemplate.update(sql,new Object[]{jsonMap.get("schedule"),jsonMap.get("kpiCode"),jsonMap.get("agentId"),jsonMap.get("ampInstId")});
        }else{
           String sql = "update TD_AVMON_AMP_POLICY set SCHEDULE = ? where KPI_GROUP = ? and AGENT_ID = ? and AMP_INST_ID = ? ";
    ...
    public List<Map<String,Object>> findAmpInstAttrGridInfo(HttpServletRequest request) {
        ...
        String listSql = SqlManager.getSql(AgentManageService.class, "findAmpInstAttrList");
	 ...
	 * @param schedule
	 */
	public Map saveVmSchedule(String agentId, String ampInstId, String schedule,String dispatchIssuedSuccess) {
	...
	public void updateVmMonitorStatus(String agentId, String ampInstId,
		...
		if("start".equalsIgnoreCase(flag)){//若是启动新的监控虚机，需要调用添加MO对象接口。
			for(String objId:objIdArray){
				...
				// modify by mark start 2014-1-23
				if(null != vmMap){
					String ip = vmMap.get("ip");
					if(!StringUtils.isEmpty(ip)){
						attrMap.put("ip", ip);
						attrMap.put("realip", ip);
					}else{
						attrMap.put("ip", "NA");
						attrMap.put("realip", "NA");
					}
				}
//				attrMap.put("ip", vmMap!=null?vmMap.get("ip"):null);
				// modify by mark end 2014-1-23
				// modify by mark start 2014-1-24 td_avmon_mo_info 中的caption--->hostName
				String hostName = vmMap!=null?vmMap.get("hostName"):null;
				attrMap.put("hostName", hostName);
//				logger.debug("--------------------------------hostName is "+hostName);
				String addReslult = this.addMo(moId, "VE_VMHOST", hostName, "VE","AGENTLESS",objId, agentId, attrMap);
				// modify by mark end 2014-1-24
	...
	public Map findAmpInstPolicyGridInfo(HttpServletRequest request) {
        ...
        if("new".equalsIgnoreCase(flag)){
        	listSql = SqlManager.getSql(AgentManageService.class, "findAmpKpiList");
        	listSql = String.format(listSql, agentId, ampInstId);
        }else{
        	listSql = SqlManager.getSql(AgentManageService.class, "findAmpInstPolicyList");
	...
	public Map findIloHostGridInfo(HttpServletRequest request) {
        ...
        String listSql = SqlManager.getSql(AgentManageService.class, "findAmpIloHostList");
        listSql = String.format(listSql, agentId, ampInstId);
	...
	public String saveIloHost(HttpServletRequest request) throws Exception{
		...
		List<String> sqlList = new ArrayList<String>();
		String mo_id = UUID.randomUUID().toString().replace("-", "");
		...
		if("new".equalsIgnoreCase(flag)){
			if(iloHostCount>0){
				...
				attrMap.put("realip", ip);
				attrMap.put("hostname", hostName);
				String addResult = this.addMo(mo_id, "HARDWARE_HP", hostName, "HARDWARE","AGENTLESS", ip, agentId, attrMap);
	 ...
	 */
	public String addMo(String moId,String typeId,String caption,String parentId,String protocol,String desc,String agentId,Map<String,String> attrMap){
		...
		if(moCount>=maxMoCount){
			...
			mo.setParentId(parentId); 
			mo.setMoId(moId);
			mo.setType(typeId);
			mo.setProtocalMethod(protocol);
			if(attrMap!=null){
	...
	public Map pushAgentAmpSchedule(HttpServletRequest request) {
        ...
        for(Map<String,String> ampMap : ampListMapF){
        	...
        	String agentId = ampMap.get("agentId");
        	String ampId = ampMap.get("ampId");
        	// add by mark start 2014-3-20
    		//判断脚本是不是已经下发了，如果没有下发，那么调度就不能下发
    		String scriptDeploySql = "SELECT status from td_avmon_amp_inst where agent_id = '"+agentId+"' and amp_inst_id='"+ampInstId+"'";
    		int status = jdbcTemplate.queryForInt(scriptDeploySql);
    		if(status == 0){
    			buff.append(bundle.getString("deployAMPfirst"));
    			continue;
    		}
    		// add by mark end 2014-3-20
        	
        	//TODO 在这里应该下发的策略信息更新,把下发的策略更新为已启用
        	String result = avmonServer.deployAmpSchedule(agentId,ampInstId);
        	buff.append(ampInstId);
    		if(result.startsWith("00")){
        		String updateAmpStatusSql = String.format(SqlManager.getSql(this, "updateAMPInstallTime"),ampInstId,ampId,agentId);
        		jdbcTemplate.update(updateAmpStatusSql);
        		// add by mark start 2014-6-17
        		String updateScheduleStatusSql = "update TD_AVMON_AMP_POLICY set status = '1' where agent_id = '"+agentId+"' and amp_inst_id = '"+ampInstId+"' and  schedule != '"+NOT_EXISTS_DATE+"'";
        		jdbcTemplate.update(updateScheduleStatusSql);
        		// add by mark end 2014-6-17
    ...
    public String addAgentMo(String moId, String agentId) {
        ...
        if(list!=null && list.size()>0){
            ...
            if(os!=null){
                ...
                attr.put("os",os);
                return this.addMo(moId, type, ip, "HOST","AGENT","Auto created for agent.", agentId, attr);
            }
        }
        return "99";//99:未知错误
    }
