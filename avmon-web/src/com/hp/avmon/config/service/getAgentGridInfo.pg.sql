...
ag.ip as "ip",
coalesce(ag.host_name,mo.value) as "hostName",
ag.last_update_time as "lastUpdateTime",
ag.last_heartbeat_time as "lastHeartbeatTime",
((extract(EPOCH from now()  -ag.last_heartbeat_time))/60)::int as "agentStatus",
((extract(EPOCH from now()  -ag.last_update_time))/60)::int as "updateStatus",
