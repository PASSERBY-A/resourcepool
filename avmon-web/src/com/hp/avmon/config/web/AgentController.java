...
@Controller
@RequestMapping("/pages/config/agentManage/*")
public class AgentController {
    ...
    public void batchPushAgentAmpSchedule(HttpServletRequest request,PrintWriter writer){
        ...
        String jsonData = JackJson.fromObjectToJson(map);
        logger.debug("batchPushAgentAmpSchedule: "+jsonData);
        writer.write(jsonData);
        writer.flush();
        writer.close();
    }
    ...
    public void pushAmpSchedule(HttpServletRequest request,PrintWriter writer){
        ...
        String jsonData = JackJson.fromObjectToJson(map);
        logger.debug("pushAmpSchedule: "+jsonData);
        writer.write(jsonData);
        writer.flush();
        writer.close();
    }
    ...
    @RequestMapping(value = "/getNormalAmpSchedule1")
    public void getNormalAmpSchedule1(HttpServletRequest request, PrintWriter writer){
        Map map = agentManageService.getNormalAmpSchedule(request);
        String jsonData = JackJson.fromObjectToJson(map);
        logger.debug("getNormalAmpSchedule: "+jsonData);
        writer.write(jsonData);
...
//            }
    ...
    private List<Map<String, Object>> findVmList(String agentId,String ampInstId){
    	String sql = "select obj_id as \"id\",obj_name as \"text\",obj_parent as \"pid\",obj_type as \"objType\"," +
				...
				"where agent_id='%s' and amp_inst_id='%s' AND OBJ_TYPE IN ('HostSystem','VirtualMachine','Datacenter','Datastore')" +
	...
	private Map<String, TreeObject> ganerateVmMap(List<Map<String, Object>> list,String flag){
		...
		for(Map<String, Object> temp : list) {
			...
			obj.setHostIp(MyFunc.nullToString(temp.get("hostIp")));
			// add by mark end
			obj.setHostStatus(MyFunc.nullToString(temp.get("hostStatus")));
			obj.setEnableFlag(MyFunc.nullToString(temp.get("enableFlag")));
...
//					"{id:'network-727',vm:'vm-1714,vm-2744,vm-2847,vm-2930,vm-2932,vm-2934,vm-3203,vm-3252,vm-3256,vm-3262,vm-3264,vm-3272,vm-3276,vm-3278,vm-3280,vm-3282,vm-3284,vm-3286,vm-3304,vm-3306,vm-3308,vm-3362,vm-3666,vm-3715,vm-3717,vm-3769,vm-3822,vm-4226',host:'host-722,host-898,host-1094,host-1099,host-1109',name:'VM Network',parent:'datacenter-2',type:'Network'}]";
			...
			logger.debug("vmlistjson is------------------------------------------------------- "+returnString);
			if(returnString==null){
				return Utils.responsePrintWrite(response,"{success:'false'}",null);
			}
			
			if (returnString.indexOf("[")<= -1) {
				return Utils.responsePrintWrite(response,"{success:'false',msg:'format error'}",null);
			}
			...
			for(int i=0;i<returnVmHosts.size();i++){
				...
				if(!Constants.VirtualMachine.equals(obj.get("type"))&& !Constants.HostSystem.equals(obj.get("type"))&& !Constants.Datacenter.equals(obj.get("type")) && !Constants.DataStore.equals(obj.get("type"))){
					continue;
				}
				...
				if(Constants.VirtualMachine.equalsIgnoreCase(type)){
				...
				String hostIp =  null;
				if(Constants.HostSystem.equals(type)){
					try {
						hostIp = obj.getString("config.virtualNicManagerInfo");	
					} catch (Exception e) {
						logger.debug("JSONObject[\"config.virtualNicManagerInfo\"] not found");
					}
				}else{
					try {
						hostIp = obj.getString("summary.guest.ipAddress");	
					} catch (Exception e) {
						logger.debug("JSONObject[\"summary.guest.ipAddress\"] not found");
					}
				}
				
				if(null==hostIp || "null".equals(hostIp)||StringUtils.isEmpty(hostIp)){
					m1.put("hostIp", "");//ip						
				}else{
					m1.put("hostIp", hostIp);//ip	
				}
				//虚拟机名称取得方式
				//1.hostname存在，则取hostname，
				//2.如果没有，就取name属性
				String hostName = StringUtils.EMPTY;
				
				try {
					hostName = obj.getString("summary.guest.hostName");
				} catch (Exception e) {
					logger.debug("JSONObject[\"summary.guest.hostName\"] not found");
				}
				
				if(StringUtils.isEmpty(hostName)){
					hostName = obj.getString("name");
				}
				
//				m1.put("hostName", obj.get("host"));//host_name->runtime.host
				m1.put("hostName", hostName);//host_name->runtime.host
				// modify by mark start 2014-1-23
				newVMList.add(m1);
			}
	...
	public String updateVmMonitorStatus(
		...
		try {
			...
			logger.debug("==============request.setAttribute(\"agentAmpInfo\",list)=========="+request.getAttribute("agentAmpInfo"));
