package com.hp.avmon.deploy.web;
...
import com.hp.avmonserver.schedule.service.IRemoteScheduleService;
...
public class DeployController {
   ...
   private AgentManageService agentManageService;
    ...
    public void monitorList(HttpServletRequest request,PrintWriter writer) throws Exception {
        Map<String, Object> map = deployService.getAmpInstanceList(request);
        String json=JackJson.fromObjectToJson(map);
        logger.debug("===================monitorList============="+json);
        writer.write(json);
        writer.flush();
        writer.close();
    }
    ...
    public void syncMem(HttpServletRequest request
        ...
        Map<String,Object> map = deployService.syncMem(type, request);
        Locale locale = request.getLocale();
        ResourceBundle bundle = ResourceBundle.getBundle("messages",locale);
        boolean result = (Boolean) map.get("success");
        if(result){
        	try {
        		logger.debug("buildAlarmTreeTable ...");
                alarmViewService.buildAlarmTreeTable(userId);
                alarmViewService.buildAlarmTreePath(userId);
                logger.debug("buildAlarmTreeTable Done.");                
    		} catch (Exception e) {
    			logger.debug(e.getMessage());
    			e.printStackTrace();
    			map.put("success", false);
            	map.put("msg", bundle.getString("synchronizationFailed"));
    		}
        }
        //构造告警树
    ...
    public void getAgentTree(HttpServletRequest request,PrintWriter writer) throws Exception {
    ...
    @RequestMapping(value = "restartAll")
    public String restartAll() throws  RemoteException, MalformedURLException, NotBoundException {
    	System.out.println("invoke success");
		String url = "rmi://16.159.216.111:9900/AvmonSchedule";		   
		IRemoteScheduleService iremoteScheduleService;
		iremoteScheduleService = (IRemoteScheduleService)Naming.lookup(url);
		String retstr = iremoteScheduleService.restartAll();
		return "";
    }

	
    @RequestMapping(value = "oneDeployMonitor")
    public void oneDeployMonitor(HttpServletRequest request,PrintWriter writer) throws Exception {
        
        String item=request.getParameter("item");
        Map map = deployService.oneDeployMonitor(item,request);
        //to-anyi:请在这里将list处理成带层级的json格式
        String json=JackJson.fromObjectToJson(map);
